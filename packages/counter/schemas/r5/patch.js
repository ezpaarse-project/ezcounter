...
const optionalItemFields = new Set([
  'Platform',
  'Title',
  'Publisher',
  'Database',
]);
const optionalHeaderFields = new Set([
  'Created',
  'Created_By',
  'Report_Name',
  'Release',
...
errorSchema.required = errorSchema.required.filter(
  (field) => field !== 'Severity'
);
// Allow Severity to be an integer
errorSchema.properties.Severity = {
  anyOf: [errorSchema.properties.Severity, { type: 'integer' }],
};
// Allow Code as string
errorSchema.properties.Code = {
  anyOf: [errorSchema.properties.Code, { type: 'string', pattern: '^[0-9]+$' }],
};
...
Object.values(r5Schema.definitions).forEach((value) => {
  const schema = value;
